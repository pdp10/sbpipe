...
- LaTeX 2013 (only for reports) [https://latex-project.org/ftp.html](https://latex-project.org/ftp.html)
...
After configuring the environment variables the next step is to install Python and R packages used by SB pipe. Two scripts 
...
packages (Copasi, Python, R, and LaTeX) are correctly found. Specifically for Python, the python scripts `pip.py` and `easy_install.py` are located inside the folder `Scripts` within the Python root directory. The path to this folder must also be added to `PATH`.
...
After configuring the environment variables directly or internally in Cygwin, the next step is to install Python and R packages used by SB pipe. Two scripts 
are provided to perform these tasks automatically. 
