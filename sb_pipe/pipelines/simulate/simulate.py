...
class Simulate(Pipeline):
	...
	logger.info("################")
	self.generate_data(model, models_dir, os.path.join(outputdir, sim_data_folder), cluster, pp_cpus, runs)
	...
	logger.info("##############")
	self.analyse_data(model[:-4], os.path.join(outputdir, sim_data_folder), outputdir, sim_plots_folder, simulate__xaxis_label)    
	...
	logger.info("##################")
	self.generate_report(model[:-4], outputdir, sim_plots_folder)
    ...
    def generate_data(self, model, inputdir, outputdir, cluster_type="pp", pp_cpus=2, runs=1):
    ...
    def analyse_data(self, model, input_dir, outputdir, sim_plots_folder, xaxis_label):   
