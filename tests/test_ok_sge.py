...
class TestIRSGE(unittest.TestCase):
    ...
    def test_stoch_simul_copasi_sge(self):
        ...
        try:
            ...
            self.assertEqual(sbpipe.main(["sbpipe", "--simulate", "sge_ir_model_stoch_simul.conf"]), 0)
        except OSError as e:
    ...
    def test_param_estim_copasi_sge(self):
        ...
        try:
            subprocess.Popen(['qstat'], stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()[0]
            self.assertEqual(sbpipe.main(["sbpipe", "--param-estim", "sge_ir_model_param_estim.conf"]), 0)
        except OSError as e:
            print("Skipping test as no SGE (Sun Grid Engine) was found.")
    ...
    def test_stoch_param_estim_copasi_sge(self):
        ...
        try:
            subprocess.Popen(['qstat'], stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()[0]
            self.assertEqual(sbpipe.main(["sbpipe", "--param-estim", "sge_ir_model_stoch_param_estim.conf"]), 0)
        except OSError as e:
            print("Skipping test as no SGE (Sun Grid Engine) was found.")
