...
Before installing sb\_pipe Python and R dependencies the following
...
On Windows platforms, these environment variables are configured as any
other Windows environment variable.

Now it is the time to install Python and R packages used by sb\_pipe.
...
``libcairo-dev``, ``libssl-dev``, ``libcurl4-openssl-dev``. After
installing these packages, ``install_rdeps.r`` must be executed again.
...
The correct installation of sb\_pipe and its dependencies can be checked
by running the following commands inside the sb\_pipe folder:
...
Before using these pipelines, a Copasi model must be configured as
follow using CopasiUI:
...
-  Save the report in the same folder with the same name as the model
   but replacing the extension .cps with .csv.
...
**it is advised** to move them into the Models/ folder so that the
Copasi (.cps) file and its associated experimental data files are stored
in the same folder. To run sb\_pipe, users need to create a
...
Working\_Folder/. This folder will eventually contain all the results
generated by sb\_pipe.

Finally, a pipeline with a certain configuration file can be executed as
follows:
...
In sb\_pipe each pipeline executes three tasks: data generation, data
...
Assuming that the configuration files are placed in the Working\_Folder
...
sb\_pipe is a relatively young project and there is a chance that some
...
information is needed. This detail data can be found in sb\_pipe log
files which are stored in ${HOME}/.sb\_pipe/logs/. When using the
mailing list above, it would be worth providing this extra information.

Issues and feature requests can also be notified using the github issue
tracking system for sb\_pipe at the web page:
https://github.com/pdp10/sb_pipe/issues.

.. |Build Status| image:: https://travis-ci.org/pdp10/sb_pipe.svg?branch=master
   :target: https://travis-ci.org/pdp10/sb_pipe
